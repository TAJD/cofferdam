...
- CD-40 lever 5, PR 2 of 5 (engine substrate, pure refactor): the `IMPORTS`/`EXPORTS` corpus slots (`cofferdam_core::graph`) are now bucketed per originating file (a new `PerFile<T>` corpus type) instead of one flat `Vec<Record>` shared by every file in the project. `Engine::finalize_and_filter`'s incremental path used to rebuild the canonical graph's delta by scanning every import/export record in the corpus and testing membership in `changed` per record (O(total records × |changed|)); it now looks up each changed file's records directly (O(|changed|)). A new `FinalizeScope` corpus slot (`Full` / `Delta { changed, removed }`) is published before Phase A finalize runs, for the incremental aggregates PR 3+ will add. `PerFile::to_vec` caches its flattened view, invalidated on every write, so the ~13 finalize-stage checks that still read the full flat view (until PR 3-5 migrate them to per-file/incremental queries) share one flatten-and-clone per finalize pass instead of paying it independently on every call — needed to hold the criterion CI gate (`bench.yml`'s `criterion compare vs base branch`) flat on `full_run_no_cache` and `single_file_edit_incremental`, which a first pre-sizing-only attempt didn't fully absorb. No check's findings behavior changes — verified via the full test suite (1322 tests) plus a real-repo A/B diff against `bestefforttools` (325 files) showing byte-identical `--no-cache` JSON output before/after.
