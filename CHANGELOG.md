...
### Changed
- The `cofferdam` binary now uses mimalloc as its global allocator. A run is allocation-dominated — one oxc arena `Allocator` per file, plus per-check `Vec`/`String` churn across the rayon run loop — so the general-purpose system allocator sits squarely on the hot path. Measured on a 5000-file synthetic repo (`scripts/gen-synthetic-repo.mjs --files 5000 --seed 42`, release build, `--no-cache`, interleaved A/B so machine drift can't favour either side, N=9 median): cold run **1622ms → 1425ms, −12.1%**, with the two distributions non-overlapping (1592–1658 before, 1394–1469 after). Findings are byte-identical. The binary grows 13,116,928 → 13,134,336 bytes (+17 KB, +0.13%), so neither the 7-platform release matrix nor npm package size is materially affected. Scoped to the binary via a default-on `mimalloc` feature on `cofferdam-cli`: the library crates stay allocator-neutral so embedders keep their own choice, and `cargo build -p cofferdam-cli --no-default-features` restores the system allocator for any environment that can't compile mimalloc's C source (CD-169).
