...
- `Design.DuplicateExportName` (CD-40, lever 5, PR 1 of 5): `finalize()` no longer re-groups every export in the project by name on every `Engine::analyze_incremental` call. Each name's collision group is now maintained incrementally — a file's exact contribution is subtracted precisely on edit/removal (never re-derived from already-mutated corpus state) and only names whose group actually changed since the last call are re-evaluated; the resulting issue is cached per group. Findings are byte-identical to a from-scratch run (verified on a real-world corpus, `bestefforttools`, plus 7 new topology-complete parity tests covering shrinking groups, primary-occurrence migration, and boundary-config changes). The other four cross-file emitters (`Warning.UnusedImport`, `Refactor.DeadExport`, `Design.OrphanExport`, `Design.ImportCycle`) remain O(all files) per call pending their own PRs.
- CD-40 lever 5, PR 2 of 5 (engine substrate, pure refactor): the `IMPORTS`/`EXPORTS` corpus slots (`cofferdam_core::graph`) are now bucketed per originating file (a new `PerFile<T>` corpus type) instead of one flat `Vec<Record>` shared by every file in the project. `Engine::finalize_and_filter`'s incremental path used to rebuild the canonical graph's delta by scanning every import/export record in the corpus and testing membership in `changed` per record (O(total records × |changed|)); it now looks up each changed file's records directly (O(|changed|)). A new `FinalizeScope` corpus slot (`Full` / `Delta { changed, removed }`) is published before Phase A finalize runs, for the incremental aggregates PR 3+ will add. `PerFile::to_vec` caches its flattened view as an `Arc<Vec<T>>`, invalidated on every write, so the ~13 finalize-stage checks that still read the full flat view (until PR 3-5 migrate them to per-file/incremental queries) share one flatten-and-clone per finalize pass and every later caller in that pass pays only an `Arc` refcount bump instead of a deep clone — needed to hold the criterion CI gate (`bench.yml`'s `criterion compare vs base branch`) flat on `full_run_no_cache` and `single_file_edit_incremental`. Two earlier attempts (pre-sizing the allocation, then caching an owned `Vec<T>` that each caller still deep-cloned) both left a small but statistically significant regression; wrapping the cache in `Arc` removes the deep clone entirely for cache hits. No check's findings behavior changes — verified via the full test suite (1322 tests) plus a real-repo A/B diff against `bestefforttools` (325 files) showing byte-identical `--no-cache` JSON output before/after.

### Fixed
- `scripts/smoke.ps1`'s `-Memory` gate now pins `RAYON_NUM_THREADS=4` for the measured run (CD-194). Rayon defaults its worker pool to the host's core count, and each worker's own bookkeeping adds to peak RSS, so the gate's growth-vs-baseline comparison conflated thread-pool size with genuine memory regressions — an unpinned run on a higher-core-count machine than the one the baseline was recorded on could fail (or pass) for reasons unrelated to any real change (confirmed: an unmodified `main` build measured +37.4% peak RSS on a 32-core machine vs +5.8% on a 4-core one). The baseline (`scripts/smoke-baseline.json`) is re-recorded against the pinned thread count.
...
- `cofferdam context`'s digest `assemble()` no longer pays a full `serde_json` serialization per item after the token budget is already exhausted — it now stops costing items once no remaining group head can fit, instead of costing (and discarding) every item down to the last one (CD-264).
- `cofferdam context --format json`'s `changed_files` list is now capped at 500 entries, with a `changed_files_truncated_from` field reporting the true count when the cap applies — previously it was emitted in full with no budget accounting, so a large branch diff could blow well past the requested `--budget` (CD-265).
- `type_host.rs` and `plugins.rs`'s `write_atomic` derived their temp-file path from the process id alone, so both modules computed the identical tmp path when materialising the same `type-host-core.mjs` into the same scripts directory in one process. A race between the two writers could rename a partially-written file into place, reopening the torn-file bug CD-266 was filed to eliminate. Each call site's tmp path now includes a discriminator unique to its writer (CD-276, CD-283).
- `type_host.rs`/`plugins.rs`'s `materialise_host_script` no longer caches an `Err` in its `OnceLock` forever. A transient write/rename failure (a full temp dir, an antivirus scanner or another process briefly holding the destination file open) used to permanently disable the type host — or every plugin — for the rest of the process, even once the underlying condition cleared. It now caches only the successful path, behind a double-checked `Mutex`-guarded init, and retries on the next call after a failure (CD-277).

## [0.4.0] - 2026-08-02

### Changed
- The disk-backed findings + whole-run cache (`.cofferdam/cache/<version>/`) is no longer net-negative at scale. On a 5000-file synthetic repo the cache made a one-file-edit re-run *slower* than a cold `--no-cache` run, and got worse every time it ran. Four causes, all fixed: (1) `run.json` retained one full-project snapshot per input-set fingerprint ever seen, so it grew by ~25 MB per edit — after eight runs it was 210 MB; `save_run` now persists only the most recently inserted entry. (2) Every findings record serialised its `content_hash` and `config_hash` as `[u8; 32]`, which serde renders as a 32-element decimal array — ~31 MB of the 50 MB file was hashes alone; they are now hex strings, hoisted so each *file* carries one instead of each file×check. (3) 133k of the 140k findings records held an empty issue list (the "this check found nothing here" fact the cache exists to record) while paying full key overhead; those are now bare `u32` indices inside their file's group. (4) Every cached `Issue` embedded its absolute path twice plus its check id and message in full; all four now reference a per-file string table by index, and `LocationRange::Bytes` serialises as a 4-element array rather than a tagged object. Additionally, a run whose cache lookups all hit now skips the rewrite entirely instead of re-serialising an unchanged file. Measured on the 5000-file synthetic repo (`scripts/gen-synthetic-repo.mjs --files 5000 --seed 42`, release build, N=7 median): warm-after-one-file-edit **2634ms → 1650ms** (and no longer degrading run over run: 2161→3192ms became a flat 1613–1743ms), no-op warm re-run **2172ms → 469ms**, cold unchanged at ~1.56s. On-disk cache 263 MB → 11 MB at 5000 files, 3.5 MB → 244 KB on projektor (476 files). Findings are byte-identical to a `--no-cache` run in every case. A pre-existing v1 cache file no longer parses and is discarded through the existing corruption path, rebuilding on the next save (CD-185).
- `Design.DuplicateTypeShape`'s `finalize()` no longer compares every collected shape against every other one. It was a literal all-pairs loop with a fresh `HashSet` allocated per pair, and at 5000 files (≈3250 collected shapes) it accounted for ~85% of total wall-clock. Three lossless changes: the field-set union/intersection is now a zero-allocation two-pointer merge over the already-sorted `BTreeMap` keys; shapes are bucketed by their `extends` clause, since a differing base already short-circuits to zero similarity; and within a bucket, shapes are sorted by field count so the count-ratio bound (`similarity ≤ min/max`, hence `similarity ≥ 0.8` requires `min/max ≥ 0.8`) reduces the candidates to a sliding window. Measured on a 5000-file synthetic repo (`scripts/gen-synthetic-repo.mjs --files 5000 --seed 42`, release build, `--no-cache`, N=5 median): **9786ms → 1622ms**, with byte-identical findings. Backed by a differential test asserting the optimized clustering matches the retained brute-force implementation across 3500 seeded-random trials (CD-180).
...
### Added
- `VariableDeclarationNode.declarations[].initSpan` (plugin AST surface): the initializer's own span, populated whenever a declarator has an initializer at all — independent of `init`, which stays `undefined` for initializer kinds the v0 wire surface doesn't model as a full node (string/numeric literals, binary expressions, ...). Lets a plugin isolate just the initializer's source text (e.g. via `SourceFile.textAt`) instead of falling back to the whole declaration's span. Includes wrapping parentheses when the initializer is parenthesized, matching how oxc reports the initializer expression's own span (CD-190).
- `SourceFile.textAt(span)` on the plugin AST surface: returns the source text covered by a `Span`, decoded correctly from its UTF-8 byte offsets. `SourceFile.text` is a JS (UTF-16) string, so `text.slice(span.start_byte, span.end_byte)` silently returns the wrong substring whenever non-ASCII content precedes the span — `textAt` does the byte-safe slice for you (CD-191).
...
- A plugin's `files.layers` scope filter silently excluded every file on a no-path scan (`cofferdam check` / `cofferdam advise` with no explicit path argument, which walks from `.`). The discovery walk's paths carried a leading `./`, which broke `layer_for`'s `strip_prefix` against the always-absolute `project_root`, so no file ever resolved a layer and every layer-scoped plugin constraint was dropped without warning. Fixed at all three call sites — the plugin scope pre-filter, the plugin manifest builder, and `cofferdam advise`'s per-file layer resolution — by resolving against the already-absolutized path before calling `layer_for` (CD-192).
- A plugin naively slicing `file.text` with a `Span`'s byte offsets (the documented, and only, way to read an AST/attribute's own text before this release) got the wrong substring whenever non-ASCII content preceded the span — `file.text` is UTF-16-indexed, `Span.start_byte`/`end_byte` are UTF-8 byte offsets. `LineView.spanFor` had the same conflation internally (it added a UTF-16 char index straight to a UTF-8 byte `lineStart`). Both are now byte-safe; see the `SourceFile.textAt` addition above for the new byte-safe accessor (CD-191).

## [0.3.13] - 2026-07-30

### Fixed
- `cofferdam check --baseline` no longer flags most pre-existing findings as `new` after a `cofferdam baseline write` with nothing changed in between. A `RunCache` hit (the default once `.cofferdam/cache` is warm — including the very next invocation after a `baseline write`) rebuilt its returned per-file text map from the raw, un-absolutized input paths, while the replayed `Issue`s it paired them with had already been stamped with the absolutized paths every fresh analyze pass uses (CD-q9f). `baseline::signature_for_issue`'s `texts.get(&issue.file)` lookup then missed for every finding, silently falling back to hashing an empty snippet — a degenerate signature that (almost) never matches the baseline's real one, so re-matching only ever "worked" for the rare finding whose true signature happened to collide. `baseline write` was never affected (it doesn't use the run cache), which is why two successive writes always agreed with each other while `check --baseline` disagreed with both. The path-absolutization step is now applied exactly once, in one shared helper, before either the cache-hit or cache-miss branch runs, so both always see the same keys (CD-153).
- `--hide-baselined` and the text-formatter `(N new, M baselined)` summary line were downstream victims of the same bug: both correctly reflect whichever `baselined` flag they're handed, so once a warm run mislabeled real matches as new, findings that should have been hidden/counted as baselined weren't. No separate formatter fix was needed — verified via a new end-to-end regression covering a cold run, a `RunCache`-warmed run, and `--hide-baselined` on the warmed run (CD-153).
...
### Fixed
- `tsconfig.json` discovery for the ts-morph type host now anchors on the analyzed target (or the resolved `cofferdam.toml`'s directory), not the invoking process's CWD — the same class of bug CD-149 fixed for `cofferdam.toml` discovery, applied to `crates/cofferdam-cli/src/type_host.rs`'s consumer in `main.rs`. `cofferdam check <target>` run from outside `<target>`'s tree previously reported "no tsconfig.json found near `<cwd>`" even when `<target>` has its own valid tsconfig.json; type-aware checks (and type-aware plugin checks) now find it correctly (CD-151).
- `Design.DuplicateExportName`'s `exempt_boundary_pairs` option now warns when a side's glob pattern fails to compile instead of failing silently. A non-compiling side (e.g. an unclosed `[` character class) previously just skipped that matcher with no diagnostic, so the side matched nothing and the boundary quietly stopped exempting anything — the user had no way to know why their configured exemption wasn't taking effect. The affected collision stays flagged (the broken side can never claim a match) and a warning names the entry and pattern (CD-150).
- `Refactor.DuplicateBlock` no longer reports a single block as a duplicate of itself. A uniform run of statements with the same shape (e.g. a field-copy chain like `target.a = source.a; target.b = source.b; ...`) canonicalises every sliding window to the same structural hash, since per-window local-identifier numbering restarts at 0 each window and property names canonicalise the same way identifiers do. `finalize`'s overlap dedup only checked a new hash-group's spans against *other* groups' already-claimed territory, never against overlapping spans within the *same* group, so those self-overlapping windows were reported as spurious duplicates of each other. Overlapping windows within one hash group are now collapsed to their earliest-starting representative before emission; a genuine cross-file duplicate of the same shape is unaffected (CD-147).
- Plugin-host script materialization is now atomic (write to a process-unique temp file, then rename into place), fixing an intermittent CI failure where concurrent `cofferdam` processes racing to overwrite the shared, deterministic `plugin-host.mjs` / `type-host-core.mjs` temp-dir path could hand a partially-written script to a concurrently-spawned `node` process — surfacing as "no such check" (metadata mode returning an incomplete check list) or "host exited successfully but never emitted its completion marker". `OnceLock`-based dedup only protects writers within a single process; every `cofferdam` invocation is a separate process, so the shared path needed genuine cross-process write safety (CD-141).
- `Design.MissingTestFile` no longer flags a class that a test exercises purely for its type identity. Error and exception classes are usually asserted from the test of whatever *throws* them — `expect(fn).toThrow(ValidationError)`, `err instanceof ValidationError`, `expect(err).toBeInstanceOf(ValidationError)` — rather than from a test file named after the class's own module, so `errors.ts` was reported as untested even with `validate.test.ts` asserting against it. A test file's `instanceof` / `toThrow` / `toThrowError` / `toBeInstanceOf` operands now count as a test signal alongside the filename-pattern rule. The credit is narrow by design: a plain import into a test (a fixture builder) still does not count, the asserted local name must resolve to a real export of the file, and resolution follows one re-export hop so a class reached through an `errors/index.ts` barrel credits `errors/validation.ts`. No new configuration; fixture in `examples/missing_test_file/errors.ts` (CD-146).
