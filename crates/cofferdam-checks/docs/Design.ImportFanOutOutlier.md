...
Scope: v1's hub exclusion is a fixed basename list (`index.ts`/`index.tsx`/`index.js`/`index.jsx`/`index.mjs`/`index.cjs`/`types.ts`/`types.tsx`) — a project with a differently-named central hub (e.g. `container.ts`, a DI container) isn't covered and may be flagged as a false positive. A file with no imports and no exports at all (e.g. a side-effect-only script) never appears in the population, since it produces no import/export graph edges — "zero fan-in and fan-out still counts" only holds for files that import or export at least something.
