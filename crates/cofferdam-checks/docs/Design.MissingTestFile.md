...
Matching: for each candidate source file, the check substitutes `{name}` (the file's stem) into every `test_match_patterns` template, resolves the result relative to the source file's own directory, and looks for it in the project's known-files universe (built from the import/export graph, same construction `Design.ImportFanOutOutlier` uses). The default templates cover same-directory and sibling-`__tests__/` conventions for both `.test.` and `.spec.` naming, `.ts`/`.tsx`. Configure `test_match_patterns` for a project with a different layout (e.g. a top-level `tests/` directory mirroring `src/`).
