...
/// CD-175 regression: `Design.MissingTestFile` writes a per-test-file
/// entry into its `TYPE_ASSERTED_NAMES` corpus slot (CD-146) but, before
/// the fix, had no `register_removable`. The stale entry is harmless on
/// a pure file *removal* (the engine's own `IMPORTS` remover already
/// drops the removed file's import records, so
/// `type_asserted_exports_by_defining_file` never rejoins the orphaned
/// entry) — it only bites on an *edit*: `analyze_incremental` calls
/// `corpus.remove_file` for changed files too, but the file's import
/// records are then immediately regenerated by pass 1, so an edit that
/// drops the `instanceof`/`toThrow` usage but keeps the import re-joins
/// against the stale (pre-edit) asserted-names entry and wrongly keeps
/// suppressing the source file's finding forever. Uses the exact
/// `examples/missing_test_file/{errors.ts,covered.test.ts}` fixture
/// shape (test file name deliberately doesn't match `errors.ts`'s
/// naming-convention pattern, so only the type-assertion index can
/// suppress the finding). Needs real on-disk paths — `oxc_resolver`
/// resolves imports against the filesystem, not the in-memory source
/// map.
#[test]
fn incremental_edit_clears_stale_type_asserted_names_entry() {
    let dir = tempfile::tempdir().expect("tempdir");
    let errors_path = dir.path().join("errors.ts");
    let test_path = dir.path().join("covered.test.ts");

    let errors_text = "export class ValidationError extends Error {}\n".to_string();
    let asserting_test_text = concat!(
        "import { ValidationError } from \"./errors\";\n",
        "test(\"keeps type identity\", () => {\n",
        "  expect(new ValidationError(\"bad\") instanceof ValidationError).toBe(true);\n",
        "});\n",
    )
    .to_string();
    let edited_test_text = concat!(
        "import { ValidationError } from \"./errors\";\n",
        "test(\"constructs\", () => {\n",
    ...
    std::fs::write(&errors_path, &errors_text).expect("write errors.ts");
    std::fs::write(&test_path, &asserting_test_text).expect("write covered.test.ts");

    let sources = vec![
        (errors_path.clone(), errors_text.clone()),
        (test_path.clone(), asserting_test_text),
    ];

    let incremental_engine = engine();
    let mut state = AnalysisState::new();
    let seeded = incremental_engine.analyze_incremental(&mut state, &sources, &[]);
    assert!(
        !full_identity(&seeded)
            .iter()
            .any(|s| s.contains("Design.MissingTestFile")),
        "errors.ts must NOT be flagged while covered.test.ts covers it via `instanceof`"
    );

    std::fs::write(&test_path, &edited_test_text).expect("edit covered.test.ts");
    let incremental = incremental_engine.analyze_incremental(
        &mut state,
        &[(test_path.clone(), edited_test_text.clone())],
        &[],
    );

    let (from_scratch, _) = engine().analyze_with_sources(vec![
        (errors_path, errors_text),
        (test_path, edited_test_text),
    ]);

    assert_eq!(
        full_identity(&from_scratch),
        full_identity(&incremental),
        "editing covered.test.ts to drop its `instanceof` usage must clear the stale \
         TYPE_ASSERTED_NAMES contribution so errors.ts's Design.MissingTestFile finding \
         reappears, matching a from-scratch analysis"
    );
    assert!(
        full_identity(&incremental)
            .iter()
            .any(|s| s.contains("Design.MissingTestFile")),
        "expected errors.ts's MissingTestFile finding to reappear after the edit dropped \
         the only type-assertion covering it"
    );
}
