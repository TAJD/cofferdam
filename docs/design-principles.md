...
Check IDs are dotted (`Category.Name`), stable, and used in: suppression comments (`// cofferdam-disable-next-line Refactor.LongAndComplex`), baseline files, SARIF rule IDs, config keys (`[checks."Refactor.LongAndComplex"]`), `cofferdam advise` output, and the docs catalogue filename.
