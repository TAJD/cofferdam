...
//   7. docs/public/llms.txt      "Current version: X.Y.Z"     (generated, cd-54sh)
//
...
const LLMS_TXT = P("docs", "public", "llms.txt");
...
function collectVersions() {
  ...
  out.push({ label: "docs/public/llms.txt", version: llmsTxtVersion() });
  ...
  if (regen) {
    ...
    process.stdout.write(
      "\nGenerated files still stale — run:\n" +
        ...
        "  cargo run -p cofferdam-cli -- gen-docs        # docs/public/checks.json + llms.txt\n" +
...
// llms.txt is generated by `cofferdam gen-docs` (build_llms_txt in
// crates/cofferdam-cli/src/gen_docs.rs) and stamps the version in its
// blockquote as "Current version: X.Y.Z." — the two ends of this contract
// reference each other (cd-54sh).
function llmsTxtVersion() {
  try {
    const m = readFileSync(LLMS_TXT, "utf8").match(/Current version: (\S+?)\.?\n/);
    return m ? m[1] : null;
  } catch {
    return null;
  }
